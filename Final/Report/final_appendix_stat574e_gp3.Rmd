...
#### row bind to build a data_sf sf object for use in our model
...
kable(data.frame(AIC = unlist(results)))

```

```{r echo=FALSE}

rm(model, cov_types, formula_list)

```

This is only any example to show one noteworthy result in this process. Some models returned unusually low AICs. Upon investigating, these very-low AIC models were actually just models whose coefficients did not converge. These models are discarded, hence the model we ultimately used.
...
Using lots of methodical trial and error, we selected the model in the below chunk by lowest AIC.
