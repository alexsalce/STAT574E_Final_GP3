...
ggmap(coconino_map) + labs(title = "Coconino County Naturally Caused Large Wildfire Incidence", subtitle = "Background realization marked with X") + theme_minimal() +
  ...
  geom_sf(data=bkg_sf_cc_plot, shape = 4, size = 0.8, color = "black", inherit.aes = FALSE)
...
# spglm_formula <- Wildfires ~ I(sqrt(distance_rd_min_prisec)) + 
...
spglm_formula <- Wildfires ~  I(sqrt(distance_rd_min_isprisec)) + 
  Precipitation_Buffered + Temp_Min_Buffered

az_wf_spcov <- spcov_initial("gaussian")
...
az_prediction_grid <- read_csv(here('data/az_prediction_grid.csv'))
