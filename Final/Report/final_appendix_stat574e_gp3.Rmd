...
rm(bkg_sf_cc_plot, wf_sf_cc_plot)
...
```{r genanimations, echo=FALSE, cache=TRUE}
...
```{r temperatureanimation, cache=TRUE}
...
temp_animation <- ggplot() +
  ...
  theme_minimal() +
  labs(title = "Predicted Values in Coconino County",
       subtitle = "Temperature Change: {frame_time}°C",
       fill = "Predicted Value") +
...
precip_animation <- ggplot() +
  ...
  theme_minimal() +
  labs(title = "Predicted Values in Coconino County",
       subtitle = "Precipitation Change: {frame_time * 100 - 100}%",
       fill = "Predicted Value") +
  transition_time(sweep_value) +
  ease_aes('linear')
...
```{r renderanimations, message=FALSE, warning=FALSE, cache=TRUE, eval=FALSE}
